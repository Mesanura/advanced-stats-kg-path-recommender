...
describe('TeacherOverview', () => {
  ...
  it('maps overview dimensions and weak points into chart options', async () => {
    ...
    const scopeSelect = wrapper.findComponent({ name: 'ElSelect' })
    scopeSelect.vm.$emit('update:modelValue', 'grade:1')
    scopeSelect.vm.$emit('change', 'grade:1')
    const algorithm = wrapper.findComponent({ name: 'ElSegmented' })
    algorithm.vm.$emit('update:modelValue', 'rule')
    algorithm.vm.$emit('change', 'rule')
    await wrapper.find('button[title="刷新"]').trigger('click')
    await flushPromises()
  })
})
